...
export interface SchematicRouteResult {
  points: Point[];
  /** False when every escalation failed and the returned HV L-bend is a
   *  known-colliding fallback (audit §4.4) — surface it, don't hide it. */
  clean: boolean;
}
 ...
 * naive router; `clean: false` marks that fallback.
 */
export function routeSchematicWireDetailed(input: {
  source: Point;
  target: Point;
  obstacles: Rect[];
  ...
  if (source.x === target.x && source.y === target.y) {
    return { points: [source, target], clean: true };
  }

  if (source.x === target.x || source.y === target.y) {
    const straight = [source, target];
    if (!pathHitsAny(straight, obstacles)) {
      return { points: straight, clean: true };
    }
  }
  ...
  if (!pathHitsAny(hv, obstacles)) return { points: hv, clean: true };
  ...
  if (!pathHitsAny(vh, obstacles)) return { points: vh, clean: true };

  const routed = hananRoute(source, target, obstacles);
  if (routed && routed.length >= 2) return { points: routed, clean: true };

  return { points: hv, clean: false };
}

/** Points-only convenience wrapper over {@link routeSchematicWireDetailed}. */
export function routeSchematicWire(input: {
  source: Point;
  target: Point;
  obstacles: Rect[];
}): Point[] {
  return routeSchematicWireDetailed(input).points;
}
