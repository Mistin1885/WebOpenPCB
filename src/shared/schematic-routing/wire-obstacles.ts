 ...
 *  - a wire sharing BOTH endpoint pins (the wire being re-routed itself) is
 *    excluded entirely; a wire sharing ONE endpoint pin (same net) is a wall
 *    except at its shared end, so the route can join it at the pin but can
 *    never run collinearly on top of it.
 *
...
> = {
  ...
  // Junction nodes sit ON wires by definition — zero body; also skipped
  // entirely in collectWireObstacles (never a routing wall).
  junction: { minX: 0, minY: 0, maxX: 0, maxY: 0 },
};
...
export function collectWireObstacles(
  ...
  for (const wire of wires) {
    const sharesSource = endpointPinIds.has(wire.sourcePinId);
    const sharesTarget = endpointPinIds.has(wire.targetPinId);
    // Sharing BOTH endpoint pins = the wire being re-routed itself (duplicate
    // pin pairs are rejected at create) — fully transparent.
    if (sharesSource && sharesTarget) continue;
    const pts = wire.pointsNm;
    const rects = wireSegmentRects(pts);
    // Same-net wires must stay reachable AT the shared pin, but their body is
    // still a wall — treating the whole wire as transparent lets the router
    // run collinearly ON TOP of it (overlapping duplicate geometry). Skip only
    // the segment(s) attached to the shared endpoint, plus any segment whose
    // inflated rect covers the shared point (short pin stubs).
    const sharedPoints: Point[] = [];
    if (sharesSource && pts[0]) sharedPoints.push(pts[0]);
    if (sharesTarget && pts[pts.length - 1])
      sharedPoints.push(pts[pts.length - 1]!);
    for (let index = 0; index < rects.length; index += 1) {
      const rect = rects[index]!;
      if (sharesSource && index === 0) continue;
      if (sharesTarget && index === rects.length - 1) continue;
      if (
        sharedPoints.some(
          (p) =>
            rect.minX <= p.x &&
 ...
 * `clean: false` flags the known-colliding HV fallback (audit §4.4).
 */
export function autoRouteWirePointsDetailed(
  projection: DesignerSchematicProjection,
  ...
  return routeSchematicWireDetailed({ source, target, obstacles });
}

/** Points-only convenience wrapper over {@link autoRouteWirePointsDetailed}. */
export function autoRouteWirePoints(
  ...
  return autoRouteWirePointsDetailed(projection, sourcePin, targetPin, wires)
    .points;
}
