...
/** True iff `p` lies ON the axis-aligned segment [a,b], endpoints included.
 *  Non-orthogonal segments always return false. Exact integer math. */
export function pointOnOrthogonalSegment(
  p: Point,
  a: Point,
  b: Point,
): boolean {
...
/** True iff `p` lies strictly INSIDE the axis-aligned segment [a,b] — on the
 *  segment but not equal to either endpoint. */
export function pointStrictlyInsideOrthogonalSegment(
  p: Point,
  a: Point,
  b: Point,
): boolean {
  return (
    pointOnOrthogonalSegment(p, a, b) &&
    !(p.x === a.x && p.y === a.y) &&
    !(p.x === b.x && p.y === b.y)
  );
}
