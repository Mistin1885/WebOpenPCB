...
describe("StreamController", () => {
    ...
    const originalStartupState = process.env.ONEMIND_STARTUP_LICENSE_STATE;
    const originalStartupCode = process.env.ONEMIND_STARTUP_LICENSE_CODE;

    afterEach(() => {
        LicenseUtil.getCurrentStatus = originalGetCurrentStatus;
        if (originalStartupState === undefined) {
            delete process.env.ONEMIND_STARTUP_LICENSE_STATE;
        } else {
            process.env.ONEMIND_STARTUP_LICENSE_STATE = originalStartupState;
        }

        if (originalStartupCode === undefined) {
            delete process.env.ONEMIND_STARTUP_LICENSE_CODE;
        } else {
            process.env.ONEMIND_STARTUP_LICENSE_CODE = originalStartupCode;
        }
    });
    ...
    it("blocks paid task creation from startup blocked state without side effects", async () => {
        process.env.ONEMIND_STARTUP_LICENSE_STATE = "blocked";
        process.env.ONEMIND_STARTUP_LICENSE_CODE = "ACCESS_BLOCKED";
    ...
    it("allows grace temporarily and denies after grace expiry state transition", async () => {
        process.env.ONEMIND_STARTUP_LICENSE_STATE = "grace";
        process.env.ONEMIND_STARTUP_LICENSE_CODE = "TOKEN_EXPIRED_GRACE";
        ...
        process.env.ONEMIND_STARTUP_LICENSE_STATE = "blocked";
        process.env.ONEMIND_STARTUP_LICENSE_CODE = "GRACE_EXPIRED";
