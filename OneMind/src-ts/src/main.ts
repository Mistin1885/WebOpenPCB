...
const resolveStartupContractVersion = (): number => {
  const raw = process.env.ONEMIND_STARTUP_CONTRACT_VERSION;
...
const resolveStartupLicenseState = (): StartupLicenseState => {
  ...
  const raw = process.env.ONEMIND_STARTUP_LICENSE_STATE;
...
const resolveStartupLicenseCode = (): string => {
  ...
  const raw = process.env.ONEMIND_STARTUP_LICENSE_CODE;
  if (raw && raw.length > 0) {
    return raw;
  }
...
const db = initializeDatabase({
  filePath: path.join(APP_DATA_DIR, "OneMind.db"),
...
const server = Bun.serve({
  ...
  fetch: async (req, server) => {
    ...
    if (url.pathname.startsWith("/api/") && url.pathname !== "/api/health") {
      const token = req.headers.get("X-OneMind-Token");
    ...
    // Root endpoint
    if (url.pathname === "/") {
      return ResponseBuilder.text("OneMind Bun Sidecar - Running");
    }
