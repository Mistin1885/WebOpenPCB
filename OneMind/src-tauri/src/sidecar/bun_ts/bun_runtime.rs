...
impl BunRuntime {
    ...
    pub async fn spawn<R: Runtime>(&mut self, app: &AppHandle<R>) -> Result<u16> {
        ...
        let (mut rx, child) = if is_dev {
            ...
            let command = app
                ...
                .env(
                    "ONEMIND_STARTUP_CONTRACT_VERSION",
                    startup_license.contract_version.to_string(),
                )
                .env("ONEMIND_STARTUP_LICENSE_STATE", startup_license.state.clone())
                .env("ONEMIND_STARTUP_LICENSE_CODE", startup_license.code.clone());
    ...
    /// Verify the sidecar binary SHA-256 hash against ONEMIND_SIDECAR_HASH env var (if set).
    ///
    /// Only compiled in release builds. If ONEMIND_SIDECAR_HASH is unset, logs a warning
    ...
    fn verify_sidecar_integrity<R: Runtime>(app: &AppHandle<R>) -> Result<()> {
        ...
        let expected_hash = match std::env::var("ONEMIND_SIDECAR_HASH") {
            Ok(h) if !h.is_empty() => h,
            _ => {
                // TODO: embed hash via build pipeline (set ONEMIND_SIDECAR_HASH in CI after bun:compile)
                warn!("ONEMIND_SIDECAR_HASH not set; skipping sidecar integrity enforcement");
                return Ok(());
            }
        };
