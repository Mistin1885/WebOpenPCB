...
impl EntitlementVerifierPolicy {
    pub fn from_env() -> Self {
        let expected_issuer = std::env::var("ONEMIND_ENTITLEMENT_ISSUER")
            .ok()
            .filter(|v| !v.is_empty())
            .unwrap_or_else(|| "onemind-license-service".to_string());
        let expected_audience = std::env::var("ONEMIND_ENTITLEMENT_AUDIENCE")
            .ok()
            .filter(|v| !v.is_empty())
            .unwrap_or_else(|| "onemind-desktop".to_string());
        let grace_period_ms = std::env::var("ONEMIND_ENTITLEMENT_GRACE_MS")
            .ok()
...
impl EntitlementVerifier {
    pub fn from_env() -> Self {
        ...
        #[allow(unused_mut)]
        let mut public_keys = std::env::var("ONEMIND_ENTITLEMENT_PUBLIC_KEYS")
            .ok()
...
mod tests {
    ...
    fn policy() -> EntitlementVerifierPolicy {
        EntitlementVerifierPolicy {
            expected_issuer: "onemind-license-service".to_string(),
            expected_audience: "onemind-desktop".to_string(),
    ...
    fn claims(now_sec: u64, jti: &str, access_status: &str, exp_sec: u64) -> EntitlementClaims {
        EntitlementClaims {
            iss: "onemind-license-service".to_string(),
            aud: "onemind-desktop".to_string(),
