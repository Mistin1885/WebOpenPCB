...
import type { ChatMetadata, MentionReference, ProjectRecord } from "@shared/types";
import { useBackendURL } from "@/contexts/BackendURLContext";
import { useMessageActions } from "@/hooks/useMessageActions";
import { useBookmarks } from "@/hooks/useBookmarks";
import type { MessageAction, MessageActionPayload } from "@/components/ChatInterface/types";
...
export function ChatScreen() {
  const { chatId, navigateToChat, chatProjectContext, navigateBackFromChat } = useNavigationStore();
  ...
  const [projectContext, setProjectContext] = useState<ProjectRecord | null>(null);
  ...
  const { setRightTopButtons } = useRegisterSidebarButtons();
  ...
  useEffect(() => {
    setRightTopButtons([
      <Button
        key="chat-media-sidebar-toggle"
        variant="ghost"
        size="icon"
        className="rounded-lg"
        aria-label={isMediaSidebarOpen ? "Close media sidebar" : "Open media sidebar"}
        aria-controls="chat-media-sidebar"
        aria-expanded={isMediaSidebarOpen}
        disabled={!chatId}
        onClick={() => setIsMediaSidebarOpen((prev) => !prev)}
      >
  ...
  }, [chatId, isMediaSidebarOpen, setRightTopButtons]);
  ...
  const { setIsStreaming, pendingModelSelection, setPendingModelSelection, pendingInitialMessage, clearPendingInitialMessage } =
    useChatStore();
  ...
  }, [chatId, resetState, loadMessages, pendingInitialMessage, clearPendingInitialMessage, activeChat, currentConfig, activeWorkspaceId, submitMessage, toolsEnabled]);
      ...
      try {
        ...
        if (data.path) {
          window.dispatchEvent(
            new CustomEvent("onemind:navigate", {
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <ChatInterface config={{
          ...
          behavior: {
            ...
            onStop: abort,
            onBack: chatProjectContext ? navigateBackFromChat : undefined,
